...
export function AnalyticsPage() {
  ...
  // 100% Real Aggregations from user's actual links
  ...
  const topLink = sortedLinks.length > 0 && (Number(sortedLinks[0].clickCount) || 0) > 0
    ? sortedLinks[0]
    : null;

  const avgClicksPerLink = links.length > 0
  ...
  return (
    <div className="analytics-page-container">
      ...
      <div className="analytics-header-row">
        <div>
          <h1 className="analytics-page-title">Analizler</h1>
          <p className="analytics-page-desc">
            Bağlantılarınızın gerçek zamanlı tıklama performansını ve durumlarını izleyin.
          </p>
        </div>
      ...
      <div className="analytics-kpi-grid">
        <div className="analytics-kpi-card highlight">
          <div className="kpi-top">
            <span className="kpi-label">Toplam Tıklama</span>
          ...
          <span className="kpi-note">Tüm linklerinize gelen gerçek tıklamalar</span>
        </div>

        <div className="analytics-kpi-card">
          <div className="kpi-top">
            <span className="kpi-label">Toplam Bağlantı</span>
          ...
          <span className="kpi-note">Oluşturulan toplam kısa link</span>
        </div>

        <div className="analytics-kpi-card">
          <div className="kpi-top">
            <span className="kpi-label">Yayında (Aktif)</span>
          ...
          <span className="kpi-note">Ziyaretçilere açık olanlar</span>
        </div>

        <div className="analytics-kpi-card">
          <div className="kpi-top">
            <span className="kpi-label">Ortalama Tıklama</span>
          ...
          <span className="kpi-note">Bağlantı başına ortalama ziyaret</span>
        </div>
      </div>

      {/* En Çok Tıklanan / Lider Link Özeti */}
      {topLink && (
        <div className="analytics-highlight-banner">
          <div className="highlight-banner-left">
            <div className="highlight-badge">
              <TrendingUp size={13} /> En Çok Tıklanan Bağlantı
            </div>
            <strong className="highlight-title">loss.tr/{topLink.slug}</strong>
          ...
          <div className="highlight-banner-right">
            <div className="highlight-clicks-box">
              ...
              <span className="highlight-clicks-label">Gerçek Tıklama</span>
            </div>
            <button
              type="button"
      ...
      <div className="analytics-panel-card" style={{ marginTop: "20px" }}>
        <div className="panel-card-head">
          <div className="head-title-row">
            ...
            <h3>Bağlantı Bazlı Tıklama Dağılımı</h3>
          </div>
          <span className="head-badge">{links.length} Kayıtlı Link</span>
        </div>

        <div className="panel-card-content" style={{ padding: "0" }}>
          {links.length === 0 ? (
            <div className="top-links-empty">
              <Link2 size={28} style={{ color: "var(--text-muted)", marginBottom: "10px" }} />
            ...
            <div className="analytics-real-table-wrapper">
              <table className="analytics-real-table">
                <thead>
                  <tr>
                    <th style={{ width: "38%" }}>Kısa Bağlantı</th>
                    <th style={{ width: "32%" }}>Hedef Adres</th>
                    <th style={{ width: "15%", textAlign: "center" }}>Durum</th>
                    <th style={{ width: "15%", textAlign: "right" }}>Tıklama</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={link.id || link.slug}>
                        <td>
                          <div className="table-link-cell">
                            ...
                            <span className="table-link-date">
                              <Clock size={11} /> {formatTimeAgo(link.createdAt)}
                            </span>
                          </div>
                        </td>
                        <td>
                          <span className="table-link-dest" title={link.destination}>
                            {link.destination}
                          </span>
                        </td>
                        <td style={{ textAlign: "center" }}>
                          ...
                          >
                            {link.status === "active" ? "Yayında" : "Duraklatıldı"}
                          </span>
                        </td>
                        <td>
                          <div className="table-clicks-cell">
                            <strong className="table-clicks-num">{clicks}</strong>
                            {totalClicks > 0 && (
                              <div className="table-clicks-share-row">
                                ...
                                <span className="table-share-text">%{sharePct}</span>
                              </div>
                            )}
                          </div>
                        </td>
